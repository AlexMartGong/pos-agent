...
9. **Heartbeat (self-discovery + self-update):** `HeartbeatTask` (`com.agent.pos.network`) reports the agent's URL to the SaaS every 5 min so the frontend can discover it dynamically.
   ...
   - Runs once at startup, then every 5 min via `ScheduledExecutorService` (daemon). 5s connect + 5s request timeout. The shared `HttpClient` is built with `.followRedirects(HttpClient.Redirect.NORMAL)` — required so the self-update download follows GitHub's 302 to the asset CDN (`release-assets.githubusercontent.com`). Without it the empty 302 body is written to the jar (SHA-256 of empty input = `e3b0c4...b855`, never matches). All failures caught and logged — never crashes the agent.
